...
public class ExKeywordsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ex_keywords);
        if (getExKeywords() != null) {
            exKeywords = getExKeywords();
        }
            ...
            public void onItemClick(AdapterView<?> parent, final View view,
                ...
                view.animate().setDuration(2000).alpha(0)
                        .withEndAction(new Runnable() {
                            @Override
                            public void run() {
                                exKeywords.remove(item);
                                dbHelper.deleteExKeyword(item);
    ...
    public void saveKeyword(View view) {
        ...
        exKeywords.add(keyword.getText().toString());
        if (adapter != null) {
            adapter.notifyDataSetChanged();
        }
    ...
    private ArrayList getExKeywords() {
        DBHelper dbHelper = new DBHelper(this);
        Settings settings = dbHelper.readSettings();
        return settings.getExKeyWords();
    }
}
